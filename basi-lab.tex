...
ORDER BY cognome DESC, nome;
	...
	\subsubsection{Operatori di aggregazione}
		...
		Questi operatori si possono usare solo in \lstinline|SELECT| e \lstinline|HAVING|; il loro uso con altri valori necessita la presenza del campo \lstinline|GROUP BY| (che NON può contenere operatori di aggregazione!) nella query. \medskip \\
...
\section{Python e Database}
	...
	\item \lstinline|isolation_level|: proprietà r/w. Modifica il livello di isolamento per la prossima transazione. Valori leciti: \lstinline|'READ UNCOMMITTED'|, \lstinline|'READ COMMITTED'|,\lstinline|'REPEATABLE READ'|, \\ \lstinline|'SERIALIZABLE'| e  \lstinline|'DEFAULT'|. \\
...
with conn :
	...
	\end{lstlisting}
	\begin{itemize}
	\item Quando si usa una connessione con il with, all’uscita del blocco viene fatto un \textbf{commit} automatico, mentre la connessione \textbf{non viene chiusa}.
	\item Quando si usa/crea un cursore con il with, all’uscita del blocco viene fatto un \textbf{close} automatico del cursore (conviene usare sempre lo stesso cursore se possibile).
	\end{itemize}
...
\end{lstlisting}
(b) (probabilmente troppi join usati)
\begin{lstlisting}
...
La prima transazione (T1) è stata scritta con livello di isolamento REPEATABLE READ per evitare anomalie Nonrepeatable reads; se infatti le due transazioni sono concorrenti, è possibile che il controllo della seconda transazione (T2) sul valore di 'prezzo' (se è = 1050) non legga determinati valori corretti.\\
...
\begin{lstlisting}
	...
	FROM BIGLIETTO b JOIN VOLIBIGLIETTO v ON 
		...
		b.numero = v.numeroVolo AND
		b.orarioPartenza = v.orarioPartenza
	WHERE b.numeroBiglietto = '1234567890123'
\end{lstlisting}
Un indice utile potrebbe essere (numeroBiglietto?).
\textbf{TODO}
...
\paragraph{Soluzione}\dotfill
\\(a) 1 indice, \lstinline|ins_aa|:
\begin{lstlisting}
	CREATE INDEX ins_aa ON Inserogato (annoaccademico, 
		crediti, modulo)	--???
\end{lstlisting}
(b) In base al piano di esecuzione conviene/non conviene
\newpage
